					...
					<article <?php comment_class(empty( $args['has_children'] ) ? '' :'parent') ?> id="comment-<?php comment_ID() ?>" itemscope itemtype="http://schema.org/Comment">
						<div class="wrapper">
							...
							<div class="commentfooter">
								...
								<?php edit_comment_link('<p class="comment-meta-item"><i class="fa fa-edit"></i></p>','',''); ?>
								<?php if ($comment->comment_approved == '0') { ?>
								<p class="check comment-meta-item"><i class="fa fa-minus-square-o"></i></p>
								<?php } else { ?>
								<p class="check comment-meta-item"><i class="fa fa-check-square-o"></i></p>
								<?php } ?>
							</div>
						</div>
